...
class Folio {
  ...
  static async findById(id) {
    try {
      ...
      return rows.length > 0 $1 new Folio(rows[0]) : null;
    } catch (error) {
  ...
  async update() {
    try {
      const sql = `
        UPDATE "Folios" 
        SET Nombre = $1, CodigoEmpleado = $1, Planta = $1, 
            EsquemaPago = $1, TipoSolicitud = $1, Descripcion = $1, 
            Prioridad = $1
        WHERE idFolio = $1
      `;
  ...
  async delete() {
    try {
      const sql = `DELETE FROM "Folios" WHERE idFolio = $1`;
